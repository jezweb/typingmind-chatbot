 ...
 * @param {string} type - 'hourly' or 'session'
 * @param {number} limit - Rate limit value
 * @returns {Promise<Object>} Status object with count and ttl
 */
export async function checkRateLimitStatus(kv, instanceId, clientId, type, limit) {
  const key = type === 'hourly' 
    ? `rate:hour:${instanceId}:${clientId}`
    : `rate:${type}:${instanceId}:${clientId}`;
  
  const current = await kv.get(key);
  const count = current ? parseInt(current) : 0;
  
  // Get TTL for hourly limits
  let ttl = null;
  if (type === 'hourly') {
    try {
      const { value, metadata } = await kv.getWithMetadata(key);
      if (metadata && metadata.expiration) {
        ttl = Math.max(0, metadata.expiration - Math.floor(Date.now() / 1000));
      } else {
        ttl = 3600; // Default to 1 hour if no metadata
      }
    } catch (error) {
      ttl = 3600; // Default to 1 hour on error
    }
  }
