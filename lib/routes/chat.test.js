...
import { handleGetInstance, handleChat } from './chat.js';
...
jest.mock('../rate-limiter.js', () => ({
  ...
  generateRateLimitKeys: jest.fn(),
  extractClientId: jest.fn(),
  createRateLimitErrorResponse: jest.fn()
}));

import { validateInstanceId, validateDomain, createResponseHeaders } from '../security.js';
...
describe('Chat Routes', () => {
  ...
  describe('handleGetInstance', () => {
    test('should return instance info for valid ID', async () => {
      ...
      getInstanceConfig.mockResolvedValue(mockInstance);
      ...
      expect(getInstanceConfig).toHaveBeenCalledWith(mockEnv.DB, 'test-instance');
    });
    ...
    test('should return 404 when instance not found', async () => {
      ...
      getInstanceConfig.mockResolvedValue(null);
    ...
    test('should handle database errors', async () => {
      mockRequest.params = { id: 'test-instance' };
      getInstanceConfig.mockRejectedValue(new Error('DB error'));
  ...
  describe('handleChat', () => {
    beforeEach(() => {
      ...
      generateRateLimitKeys.mockReturnValue({
        hourlyKey: 'rate:hour:test',
        sessionKey: 'rate:session:test'
      });
      
      extractClientId.mockReturnValue('client-123');
    });
    ...
    test('should handle valid chat request', async () => {
      ...
      checkAndUpdateRateLimit.mockResolvedValue({ allowed: true });
    ...
    test('should handle rate limit exceeded', async () => {
      ...
      validateDomain.mockResolvedValue(true);
      checkAndUpdateRateLimit.mockResolvedValue({ 
        allowed: false,
        retryAfter: 3600
      });
      
      const mockErrorResponse = new Response('Rate limit exceeded', { status: 429 });
      createRateLimitErrorResponse.mockReturnValue(mockErrorResponse);
